...
#image, edged = read_and_preproces('12.JPG')
...
name_array=(args.filename).split('.')
image, edged = read_and_preproces(args.filename)  
image = find_object_in_pix(image, edged, args.thresh1)
cv2.imwrite('./output/'+name_array[-2]+'.'+name_array[-1], image)
cv2.imshow('kjdj', image)
